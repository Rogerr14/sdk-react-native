...
export default class InterceptorHttp{
    ...
    constructor({endpoint, methodHttp, queryParams, secretCode, secretKey, body }:interceptorHttp){
    ...
    public async request<T>(): Promise<T>{
        ...
        try {
            ...
            switch(this.methodHttp){
                case 'GET':
                    return await this.client.get<T>(this.endpoint, {params: this.queryParams});
                case 'POST':
                    return await this.client.post<T>(this.endpoint, {params: this.queryParams}, this.body)
                default:
                    return await this.client.get<T>(this.endpoint, {params: this.queryParams});
            }
        } catch (error:any) {
